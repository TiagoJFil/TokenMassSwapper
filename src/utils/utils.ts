import { KRC20TokenState } from "../services/kaspa/dtos/KRC20/KRC20TokenInfoDTO"
import { TokenFullyMintedException, TokenNotMintableException } from "../services/exceptions"


export function parseDecimals(value: string | number, decimalCount: string | number): number {
  return Number(value) / Math.pow(10, Number(decimalCount))
}

export function checkKRC20TokenState(state: KRC20TokenState) {
    switch(state) {
