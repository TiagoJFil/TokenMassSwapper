...
export class WalletController {
  ...
  async createUserWallet(
    @Body() newUserData: CreateUserInput,
  ): Promise<UserWalletCreateOutput> {
    const user = await this.userService.createUser(newUserData.platform_id,newUserData.platform_name);
    const userWallet = await this.walletService.createUserWallet(user.id);

    return new UserWalletCreateOutput(
      user.id,
      userWallet.address,
      userWallet.mnemonic,
    );
  ...
  async getUserWalletInfo(
    ...
    const walletInfo =
      await this.walletService.getUserPublicWalletInfo(user_id);
    const walletBalances = await this.cardanoTokenService.getWalletBalances(
      walletInfo.address,
    );
    // @ts-ignore
    return new OutputDTOs(
      walletInfo.address,
      walletBalances.ada,
